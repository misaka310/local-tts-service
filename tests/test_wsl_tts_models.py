...
EXPECTED_MODELS = {
    ...
    "fish_s2_pro": ("fishaudio/s2-pro", "fish_s2_pro"),
}
...
def test_wsl_bridge_maps_only_supported_models() -> None:
    ...
    assert environment_key_for_model("fish_s2_pro") == "fish_s2_pro"
    assert environment_key_for_model("indextts_2_5") == "indextts_2_5"
...
def test_wsl_bridge_maps_optional_asmr_models() -> None:
    from scripts.wsl_tts_bridge import environment_key_for_model
...
def test_asmr_requests_allow_no_reference_and_preserve_instruction(tmp_path: Path) -> None:
    from scripts.wsl_tts_infer import load_request
