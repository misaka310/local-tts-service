...
Local TTS Serviceは、モデル、参照音声、推論runtime、汎用TTS API、起動と停止など、再利用可能な音声生成基盤を所有します。利用側アプリ固有のセリフ、カテゴリ、表示資産、受入用ハッシュ、リリース契約、成果物配置や同期処理は所有しません。

外部アプリは公開APIと公開された起動契約だけを利用し、製品固有の受入・表示・配布を自身で所有します。特定の下流アプリを前提にした設定、generator、同期処理、テスト契約をこのリポジトリへ追加しないでください。
...
`scripts/persistent_rvc_service.py` は、低遅延の連続利用向けにRVCモデル、HuBERT、RMVPEを1プロセスへ保持する汎用HTTPサービスです。設定はGit対象外の `config/rvc-persistent.local.json`、公開例は `config/rvc-persistent.example.json` に置きます。利用先固有のUI、ブラウザ、端末、再生制御、マイク制御は持ちません。詳細は `docs/persistent-rvc-service.md` を参照してください。
