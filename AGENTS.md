...
- 作業完了時は通常repositoryを `main` に戻し、永続ブランチは `main` 1本を正規状態とする。一時的なfeature/review/WIP/CodexPro作業ブランチや不要worktreeを残さない。
- 新しいTTS/RVCモデルは `docs/model-onboarding.md` を完了条件にする。利用条件は「使い方」のライセンス欄へ集約し、商用区分を明示する。同じ配布モデル/チェックポイントの実行プロファイルは同じ `licenseGroup` にまとめ、別画面へ勝手にライセンス表示を増やさない。
...
- 仕様の正本: `README.md` と対応する `docs/`
- 仕様変更時は、実装前に正本を更新し、実装と検証を同じ変更で更新する。
